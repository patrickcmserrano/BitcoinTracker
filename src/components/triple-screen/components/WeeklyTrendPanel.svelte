<script lang="ts">
    ...
    import type { Trend, MACDResult } from "../types";

    export let trend: Trend;
    export let macd: MACDResult | null;
    export let ema17: number | null;
    export let exclusionRule: string;
...
<div class="timeframe-card oceano">
    ...
    {#if macd && ema17}
        <div class="indicator-values">
            <div class="indicator-row">
                <span>MACD:</span>
                <span class="value">{macd.MACD.toFixed(2)}</span>
            </div>
            <div class="indicator-row">
                <span>Signal:</span>
                <span class="value">{macd.signal.toFixed(2)}</span>
            </div>
            <div class="indicator-row">
                <span>Histogram:</span>
                <span
                    class="value {macd.histogram > 0 ? 'positive' : 'negative'}"
                >
    ...
    <div class="rule-box">
        <strong>Regra de Ouro #2:</strong>
        {exclusionRule || "Calculando tendência..."}
    </div>

    <div class="chart-preview">
        <CandleChart {symbol} interval="1w" activeTimeframe="1w" />
    </div>
</div>
