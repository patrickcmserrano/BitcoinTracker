  ...
  {:else}
    ...
    <div class="crypto-sidebar">
      ...
      <div class="crypto-list-vertical">
        {#each cryptos as crypto}      
          <button 
            class="crypto-btn-vertical {$selectedCrypto.id === crypto.id ? 'active' : ''}"
            style="--crypto-color: {crypto.color}"
            on:click={() => handleSelect(crypto)}
            title={$_(`crypto.${crypto.id}.description`)}
          >
...
<style>
  ...
  .crypto-btn-vertical.active {
    background: linear-gradient(135deg, var(--crypto-color), color-mix(in srgb, var(--crypto-color) 70%, #000000));
    color: white;
    border-color: var(--crypto-color);
    font-weight: 900;
    box-shadow: 0 6px 16px rgba(0, 0, 0, 0.6), 0 0 16px var(--crypto-color);
